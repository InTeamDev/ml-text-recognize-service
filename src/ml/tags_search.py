...
class KeywordService:
    ...
    def normalize_keywords(self, input_keywords):
        ...
        for keyword in input_keywords:
            if self.morph.parse(keyword)[0].tag.POS in self.tokens:  # type: ignore
                keywords.add(self.morph.parse(keyword)[0].normal_form if len(keyword.split()) == 1 else keyword)
        return keywords
